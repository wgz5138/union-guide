/**
 * 透析印藥水 LINE 小幫手 v5 — Apps Script
 * ════════════════════════════════════════════════════════
 *  功能①：自動收 userId（LINE webhook）
 *  功能②：每天自動發提醒 ★改：提醒落在「印日期的前一個上班日」（跳過週日/休診日）
...
function sendReminders() {
  ...
  var todo = {};   // 姓名 → {areas:[], printStr:"yyyy-MM-dd"}

  for (var r = 1; r < rows.length; r++) {
    var pStr = normDate_(rows[r][iD], tz);
    var pd = parseYmd_(pStr);
    if (!pd) continue;
    var remind = prevWorkday_(pd, tz);                       // 印日期的前一個上班日
    if (Utilities.formatDate(remind, tz, "yyyy-MM-dd") !== todayStr) continue;  // 今天才要提醒的才處理

    var nm2 = String(rows[r][iN]).trim(), area = String(rows[r][iZ]).trim();
    if (!nm2) continue;
    if (!todo[nm2]) todo[nm2] = {areas: [], printStr: pStr};
    if (area && todo[nm2].areas.indexOf(area) < 0) todo[nm2].areas.push(area);
  }
  ...
  for (var name in todo) {
    ...
    var info = todo[name];
    var pdObj = parseYmd_(info.printStr);
    var md = Utilities.formatDate(pdObj, tz, "M/d");
    var zone = info.areas.join("、");
    var zpart = zone ? ("「" + zone + "」") : "";
    pushLine_(uid2, "🔔 記得 " + md + " 要印" + zpart + "藥水喔！🙏");
    sent++;
  }
  Logger.log("提醒完成：發 " + sent + " 人，缺 userId " + miss + " 人（提醒日 " + todayStr + "）");
}
