...
def parse_sheet(path, sheet):
    ...
    _h1, _h2, _hsrc = week_hint(path, sheet)
    ...
    # 第五輪（v3.51，2026-08-10真實事件）：整份檔案的日期格全部完整合法（例如
    # 「2026/07/27」），前面四層完全不會被觸發（第一層就直接成功）——但這份檔案
    # 根本是另外兩週前的舊資料，檔名/分頁名寫「115.08.10~115.08.16」，表格裡卻是
    # 「2026/07/27~2026/08/02」。這種「內容看起來合法、但整份對到別週去了」的情況，
    # 過去完全沒有任何檢查，全靠玉繡肉眼比對格子內容才發現，屬於僥倖。
    # 這裡不管前面哪一層解出日期，只要有解出任何一個真實日期、且檔名/分頁名也能
    # 給出參考日期，就比對兩者差距——差超過3天（正常同一週的第一欄跟檔名起始日
    # 最多差1~2天，例如班表偶爾從週二才開始），視為極可能給錯檔案。
