...
- **⚠ 待查護欄**：引用法規庫外條號必須標 `⚠ 待查`
- **範例**：加班費補發爭議（`🧪` 帶入）
- **📖 記憶訓練**（第 6 區）：50 張法條情境卡（含醫療法§99/§83 醫療糾紛調解、刑法§309/§304/§305 職場霸凌刑告、勞基法§39國定假日調移舉證責任、性平法§15/§17產檢陪產假與留停復職、勞基法§24/§30加班規避手法系列——雇主設認定門檻/先刷卡再做事/異常申覆制度/特休代排/休息時間隨傳隨到/下班後line交接算隱性加班），每張含白話說明、典型情境、記憶口訣；🔊 單張朗讀／📻 連續播放（Radio 模式，iOS 鎖屏繼續，MediaSession 控制上一張/下一張/暫停，且**會記住上次播到哪張**，下次點📻自動接續播放）；🔎 查判決（直連司法院）；法律領域**不含智財**（智財法條仍在法規庫可手動查）
  - **語音朗讀機制（2026071309 改版）**：40+ 張卡是固定文字，改成**預先錄好的真人語音檔**存在 `mem-audio/00.mp3`～（對應 `MEMO_CARDS` 陣列順序索引，兩位數補零，只能在陣列**尾端新增**、不能中間插入，否則既有音檔索引全部錯位），不再即時呼叫外部 TTS API。原本用 `translate.googleapis.com/translate_tts`（Google 非官方端點）即時合成，會因為該端點不穩定/被防火牆擋而卡住或暴衝跳卡；現在只在**產生音檔當下**（開發階段，跑 `tools/gen-mem-audio.py`）呼叫它，使用者端完全不必連線，播放後由 sw.js 現有的「非 HTML 資源快取優先」策略自動快取離線可用。程式面：`memAudioUrl(card)` 由 `MEMO_CARDS.indexOf(card)` 算出檔名；`playTTS(text,onEnd,onCancel,audioUrl)` 有給 `audioUrl` 就播檔案（6 秒逾時防呆**只防完全沒開始播放**，一旦 `playing` 事件觸發就解除逾時、不會在播到一半時被強制打斷——這裡曾經寫錯導致每張卡播幾秒就被腰斬，已修掉），播不出來才退回 `speechSynthesis`；沒給 `audioUrl` 就直接用手機內建 `speechSynthesis`（用於 AI 即時生成、內容每次不同、無法預錄的情境卡，例如 `#ai-card-tts`／`.law-tts` 法規庫朗讀）。**新增/修改記憶卡文字後必須跑 `tools/gen-mem-audio.py --force` 重新產生對應的 mp3**，否則播放的還是舊語音內容。
  - **連續播放背景卡住 bug（已修）**：原本播完一張靠 `setTimeout(radioPlayCurrent,900)` 排程下一張，但手機鎖屏/切到背景時瀏覽器會凍結 `setTimeout`（音檔本身靠 MediaSession 還能播，計時器卻不會跑），導致播完一張就卡住要手動重點。已改成在 `onEnded` 內直接同步呼叫 `radioPlayCurrent()`，不依賴計時器。
  - **接續播放進度**：`radioPlayCurrent()` 每次換卡會把 `MEMO_CARDS.indexOf(card)`（全域索引）存進 `localStorage.law_radio_progress`；播完全部 45 張會自動清掉。下次按📻連續播放，若有存進度會自動切回「全部」分類並從該張接續播（toast 提示接續到哪張），沒有存進度才從第 1 張開始。
