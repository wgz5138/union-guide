...
- **⚠ 待查護欄**：引用法規庫外條號必須標 `⚠ 待查`
- **範例**：加班費補發爭議（`🧪` 帶入）
- **📖 記憶訓練**（第 6 區）：40 張法條情境卡（含醫療法§99/§83 醫療糾紛調解、刑法§309/§304/§305 職場霸凌刑告），每張含白話說明、典型情境、記憶口訣；🔊 單張朗讀／📻 連續播放（Radio 模式，iOS 鎖屏繼續，MediaSession 控制上一張/下一張/暫停）；🔎 查判決（直連司法院）；法律領域**不含智財**（智財法條仍在法規庫可手動查）
  - **語音朗讀機制（2026071309 改版）**：40 張卡是固定文字，改成**預先錄好的真人語音檔**存在 `mem-audio/00.mp3`～`39.mp3`（對應 `MEMO_CARDS` 陣列順序索引，兩位數補零），不再即時呼叫外部 TTS API。原本用 `translate.googleapis.com/translate_tts`（Google 非官方端點）即時合成，會因為該端點不穩定/被防火牆擋而卡住或暴衝跳卡；現在只在**產生音檔當下**（開發階段，跑 `tools/` 下的一次性腳本）呼叫它，使用者端完全不必連線，播放後由 sw.js 現有的「非 HTML 資源快取優先」策略自動快取離線可用。程式面：`memAudioUrl(card)` 由 `MEMO_CARDS.indexOf(card)` 算出檔名；`playTTS(text,onEnd,onCancel,audioUrl)` 有給 `audioUrl` 就播檔案（3 秒逾時防呆，播不出來才退回 `speechSynthesis`），沒給就直接用手機內建 `speechSynthesis`（用於 AI 即時生成、內容每次不同、無法預錄的情境卡，例如 `#ai-card-tts`／`.law-tts` 法規庫朗讀）。**新增/修改記憶卡文字後必須重新產生對應的 mp3**，否則播放的還是舊語音內容。
