...
- **成本效益權衡題＝「攤開算帳」版（標準與偏好的正解，2026-06-18 定案）**：RCT 第9題、SR 第10題標 `costben:true`。`qNote(q)` 要 AI **把帳攤開寫進 rationale**：(1) 本篇好處（效果量）(2) 本篇壞處/副作用 (3) 同時算「病人傷害」與「金錢成本」兩筆——低價耗材（如一瓶 2% 消毒劑）vs 它預防的併發症（一次感染＝病人受苦＋抗生素療程＋延長住院＋風險），落差好幾個量級＝判斷依據。據此給是/否/無法判斷：感染預防類只要本篇有效且無明顯嚴重傷害，權衡後**通常「是」**；但若顯示顯著傷害/無效/缺資料則給否/無法判斷——**結論要從本篇推出來、且算給使用者看，不是盲目蓋章**。⚠️ 演進教訓：「感染一律傾向是」（盲蓋）→ 使用者指出工具該守標準 → 我過度修正成「中立不表態」（抹掉真實經濟證據，也錯）→ 使用者用「2% 消毒劑 vs 抗生素療程＋病人傷害」點破 → 定案「攤開算帳、推理透明、結論偏向值得但須本篇支持」。同時忠於 CASP（要推理）、健康經濟學（成本/傷害落差是真證據）、使用者「要看得到推理才能判斷」。
...
- **2026-06-20 · 版號 14（BUILD/ver.txt/sw CACHE = `2026062014`）** — 本輪大改收尾。重點：
  - 評讀錨定 CASP 官方＋標準 EBM 方法學（每題看什麼以金標準為準，不憑喜好取捨）。
  - 統計判讀全套（p/CI/I² ＋ ARR/RRR/NNT、異質性整套、發表偏差、ITT、檢力、GRADE、MCID、診斷指標）。
  - 成本效益題＝「攤開算帳」（好處/病人傷害/金錢成本算給使用者看；前提成立就斬釘截鐵判，不擺盪）。
  - 「無法判斷」收緊（只用於資訊不足，正反證據要權衡）；區分研究間異質性 vs 組內同等對待。
  - 每題答案附白話逐句解釋（交報告用）；⚠待查自我標記；原文螢光筆；報告可放大字級。
  - 匯出加 📝 Word（可編輯）＋👁 預覽；🗑 一鍵清除；版本號移到頂端＋🔄 檢查更新。
  - 類型**預設改為「🤖 我不確定→AI 判斷」**；選錯檢核表時報告頂端跳紅色 `typeWarning` 防呆。
  - 周邊：掃碼分享頁 share.html（含回饋 QR）、回饋 Google 表單一鍵建立腳本、PWA、離線 sw.js、自動更新偵測。
- **版號規則**：`BUILD`／`ver.txt`／`sw.js CACHE` 三者每次部署同步 +1；「版號」是累計更新次數（只增不減），日期是最後更新日（部署時記得改成當天真實日期，別讓它落後）。三支讀 ver.txt 的工具（evidence／search／lawyer）BUILD 要一致。
- **部署慣例**：在 feature 分支 commit→push，再把工具檔 `git checkout` 套到 main→push（GitHub Pages 服務 main）。避開另一個 session 的樂透/finance 等檔，互不干擾。
