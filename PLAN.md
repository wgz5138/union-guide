...
- **一鍵更新（`law-update-box`）三條路**：① 主推「📋 複製更新指令給 AI」→ 貼給任何 AI 依官網吐出 JSON 格式最新法條 → 貼回「匯入」框（`importLaws()`，同法規名＋條號**覆蓋更新**、去掉快照標記）——**保證可用、跟更新 CASP 題庫同模式**；② 一鍵開官方 law.moj.gov.tw 自行核對；③「🧪 試著直接抓」（`law-try-fetch`，預期被 CORS 擋，擋住就引導走①，誠實顯示）。
...
- `lawyer.html` — ⚖️ AI 律師（第三批，獨立檔，勞動法主場；內建法條快照＋一鍵更新＋多輪辯論＋證據羅列）
